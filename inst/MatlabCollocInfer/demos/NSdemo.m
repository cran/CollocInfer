...
res0 = Profile_LS(NSfn.fn, tobs, yobs, coefs0, pars0, basisobj, ... 
                  lambda, [], more);

res0.pars  %  display the parameter estimates

sum(res0.res)  % display the final function value
...
Devec5 = eval_fd(tobs, DEfd5);

%  compare fits to the data

subplot(1,1,1)
plot(tobs, yobs, 'o', ...
     tobs, Devec1, '-', tobs, Devec2, '-', tobs, Devec3, '-', ...
     tobs, Devec4, '-', tobs, Devec5, '-') 
xlabel('Time (hours)') 
ylabel('Groundwater level (metres)')
